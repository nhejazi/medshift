...
stoch_est_ipw <- function(data,
  ...
  # compute IPW estimating equation and conservative standard error
  d_ipw <- (((g_shifted / e_pred) / mean_weights) * data$Y) - ipw_estimate
  var_ipw <- stats::var(d_ipw) / data[, .N]

  # output
  ipw_est_out <- tibble::as_tibble(list(
    param = "decomp",
    est = ipw_estimate,
    stderr = sqrt(var_ipw)
    #eif = d_ipw
  ))
  attr(ipw_est_out, "type") <- "ipw"
  attr(ipw_est_out, "param") <- "stochastic"
  attr(ipw_est_out, "eif") <- d_ipw
  return(ipw_est_out)
}
...
stoch_est_sub <- function(data,
  ...
  # output
  gsub_est_out <- tibble::as_tibble(list(
    param = "decomp",
    est = estim_sub,
    stderr = NA_real_
  ))
  attr(gsub_est_out, "type") <- "gsub"
  attr(gsub_est_out, "param") <- "stochastic"
  attr(gsub_est_out, "eif") <- rep(NA_real_, data[, .N])
  return(gsub_est_out)
}
...
stoch_est_onestep <- function(data,
  ...
  est_over_delta <- lapply(seq_along(delta), function(iter) {
    ...
    onestep_est_out <- tibble::as_tibble(list(
      param = "decomp",
      est = estim_onestep_param,
      stderr = sqrt(estim_onestep_var)
      #eif = (estim_eif - estim_onestep_param)
    ))
    attr(onestep_est_out, "type") <- "onestep"
    attr(onestep_est_out, "param") <- "stochastic"
    attr(onestep_est_out, "eif") <- estim_eif - estim_onestep_param
    return(onestep_est_out)
  })

  # output
  if (length(delta) == 1) {
    return(est_over_delta[[1]])
  } else {
