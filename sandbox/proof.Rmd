...
plan(multiprocess)
set.seed(429153)
options(scipen = 999)  # has scientific notation ever annoyed you? me too.
```
...
# print extracted point estimate
...
## Diagnostics for likelihood components
...
p_hist_g_natural <- ggplot(as_tibble(g_est$g_natural), aes(x = value)) +
  ...
  xlab(TeX("$\\hat{g}(a | w)$")) +
  ggtitle(TeX("Histogram of $\\hat{g}(a | w)$")) +
  theme_bw()
p_hist_g_natural

p_hist_g_shifted <- ggplot(as_tibble(g_est$g_shifted), aes(x = value)) +
  geom_histogram(fill = "red", alpha = 0.3) +
  xlab(TeX("$\\hat{g}_{\\delta}(a + \\delta | w)$")) +
  ggtitle(TeX("Histogram of $\\hat{g}_{\\delta}(a + \\delta | w)$")) +
  theme_bw()
p_hist_g_shifted
```
...
p_hist_e <- ggplot(as_tibble(e_est$e_pred), aes(x = value)) +
  geom_histogram(fill = "blue", alpha = 0.3) +
  xlab(TeX("$\\hat{e}(a | z, w)$")) +
  ggtitle(TeX("Histogram of $\\hat{e}(a | z, w)$")) +
  theme_bw()
p_hist_e
```
...
p_hist_m <- ggplot(as_tibble(m_est$m_pred), aes(x = value)) +
  geom_histogram(fill = "darkgreen", alpha = 0.3) +
  xlab(TeX("$\\hat{m}(a, z, w)$")) +
  ggtitle(TeX("Histogram of $\\hat{m}(a, z, w)$")) +
  theme_bw()
p_hist_m
```
