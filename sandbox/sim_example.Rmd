---
title: "Proof of Concept: Implementing Causal Mediation Analysis for Stochastic Interventions"
...
# fit and predict
g_fit_sl <- sl_lrn$train(g_task)
g_pred <- g_fit_sl$predict()

# directly computed the shifted propensity score
g_delta_pred <- (delta_shift * g_pred) / (delta_shift * g_pred + 1 - g_pred)
```

Let's take a quick look at our predictions, to check stability:

```{r plot_stability_g, out.width = '75%'}
p_hist_g <- ggplot(as_tibble(g_delta_pred), aes(x = value)) +
...
# fit and predict
e_fit_sl <- sl_lrn$train(e_task)
e_pred <- e_fit_sl$predict()
...
p_hist_e <- ggplot(as_tibble(e_pred), aes(x = value)) + geom_histogram() +
              theme_bw() + xlab("est. e(a | z, w)")
p_hist_e
```
...
```{r compute_m}
# make task for the outcome regression
m_task <- sl3_Task$new(
  data = data,
  outcome = "Y",
...
# fit and predict
m_fit_sl <- sl_lrn$train(m_task)
m_pred <- m_fit_sl$predict()
```

Let's take a quick look at our predictions, to check stability:

```{r plot_stability_m, out.width = '75%'}
p_hist_m <- ggplot(as_tibble(m_pred), aes(x = value)) + geom_histogram() +
              theme_bw() + xlab("est. m(a, z, w)")
p_hist_m
```
...
```{r substitution_est}
theta_sub <- 0
for (a in unique(data$A)) {
    m_this_a <- m_pred[data$A == a]
    g_this_a <- g_delta_pred[data$A == a]
    theta_sub_this_a <- mean(m_this_a * g_this_a)
    print(paste("Est. for A =", a, "is", theta_sub_this_a))
...
theta_re <- mean((g_delta_pred / e_pred) * data$Y)
theta_re
```
...
# Simple Example with Continuous Intervention

```{r make_dens_sl}
# SL learners for conditional densities, used for the intervention density fit
...
# baseline covariate -- simple, binary
W <- as.matrix(replicate(n_w, rbinom(n_obs, 1, p_w)))

# create treatment based on baseline W
A <- as.numeric(rnorm(n_obs, mean = tx_mult * W, sd = 1))

# mediators to affect the outcome
Z_1 <- (abs(tanh(A)) + sin(A) - 1) / 2 + W[, 1]
Z_2 <- (A + 1)^2 - W[, 2]
Z_3 <- (A - 1) / 2 + W[, 3]

# create outcome as a linear function of A, W + white noise
Y <- abs(Z_1) + Z_2 - Z_3 + A - rowSums(W) + rnorm(n_obs, mean = 0, sd = 1)

# full data structure
data <- as.data.table(cbind(Y, Z_1, Z_2, Z_3, A, W))
setnames(data, c("Y", paste("Z", 1:3, sep = "_"), "A",
                paste("W", seq_len(dim(W)[2]))))
head(data)
```
...
$$\theta_0 = \int b_0(a, w) g_{0,\delta}(a \mid w) p_0(w) d\nu(a, w),$$
for which a TML estimator may be computed, e.g., by using
https://github.com/nhejazi/txshift.

```{r est_txshift}
# let's compute theta_0 by fitting its TMLE
theta_tmle <- tmle_txshift(W = W, A = A, Y = Y, delta = delta_shift,
...
# print summary and pin point estimate
summary(theta_tmle)
theta_tml_est <- theta_tmle$param_est
```
